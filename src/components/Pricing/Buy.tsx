...
export default function Buy({ isMuseAI }: { isMuseAI?: boolean }) {
    ...
    const getPriceRender = useCallback((price: string | EMuseProductType) => {
        if (pricingMap && typeof price !== 'string') {
            let productInfo = pricingMap.get(price)
            let res = productInfo?.price
            if (!res) return '-'
            // 国内团队版 和 海外按年支付，显示月价格
            if ((isInChina && currentPlan === PlanType.team) || (!isInChina && currentBillingType === BillingType.yearly)) {
                res = res / 12
            }
            ...
            <FlexColContainer className="mt-[30px] w-full items-center px-5 font-euclid md:px-[74px]">
                <FlexRowContainer className='mb-6 w-full justify-between'>
                    ...
                    <div
                        ...
                        onClick={() => {
                            window.open(`${process.env.NEXT_PUBLIC_MUSEAI_URL}/settings/checkout/points?source=MuseDAM`)
                        }}
                    >
                ...
                {(!isInChina && !isMuseAI) ? <div className='w-full'>
                    {/* Billing Frequency */}
                    <RadioGroup.Root
                        className="relative my-[30px] flex w-fit items-center justify-start space-x-6"
                        defaultValue={currentBillingType}
                    >
                        {billingTypes.map((item) => {
                            const { type, label } = item
                            return <div className="flex items-center" key={type}>
                                <RadioGroup.Item
                                    className={cn(
                        ...
                        <FlexColContainer className='absolute bottom-2 right-0'>
                            <div className='text-[13px] font-medium leading-[18px]'>{t("pricing.plan.total-saved", { val: "20%" })}</div>
                            <Image src="/assets/Pricing/line.svg" alt="line" width={23} height={20} />
                        </FlexColContainer>
                    </RadioGroup.Root>
                </div> : <div className='h-[50px] w-full' />}
                {/* Pricing Cards */}
                <div className="grid w-full gap-6 lg:grid-cols-4">
                    {billingMenu[currentPlan].map((plan) => {
                        const { key, isHighlight, linkText, buttonType, summary, period, uncheckSummary, price, des, desFn } = plan
                        const priceKey = isInChina ? price : plan[currentBillingType];
                        const renderPrice = getPriceRender(priceKey)
                        const priceDes = desFn?.(currentPlan === PlanType.personal ? renderPrice : ((pricingMap?.get(priceKey)?.price || 0) / 100)) ?? des
                        return <div className={cn('relative border px-6 pb-6 pt-[30px]',
                            isHighlight && 'border-none shadow-[0px_10px_30px_0px_#7E6CE81F]')}
                            key={key}
                        >
                            {isHighlight && <div className='absolute left-0 top-0 h-[3px] w-full' style={{
                                background: 'linear-gradient(257.16deg, #7E6CE8 10.55%, #CF6CE8 89.44%)'
                            }} />}
                            <div>
                                <h3 className="text-5 flex items-center gap-1 font-medium leading-6">{plan.title}
                                    {plan.icon && <Image src={plan.icon} width={16} height={16} alt='icon' />}
                                </h3>
                                <p className="mt-1 font-mono text-[13px] leading-[18px] text-[#95989F]">{plan.description}</p>
                            </div>
                            <div className="mt-[10px] flex items-center gap-[6px]">
                                {period && <span className="text-[12px] font-medium ">{isInChina ? '￥' : '$'}</span>}
                                <span className={cn(" font-medium leading-[32px]", !period ? 'text-[28px]' : 'text-[32px]')}>{renderPrice}</span>
                                {period && <span className="font-mono text-[12px] leading-4 text-[#262626] ">{period}</span>}
                            </div>
                            {priceDes && <span className='mt-[6px] font-mono text-[14px] leading-[20px] text-[#95989F]'>{priceDes}</span>}
                            <Button
                                ...
                                onClick={() => {
                                    if (buttonType === 'contact') {
                                        ...
                                        window.open(`${process.env.NEXT_PUBLIC_MUSEAI_URL}/settings/checkout/plan?product=${priceKey}`, '_blank')
                                    } else if (buttonType === 'try') {
                                        window.open(MUSEDAM_LOGIN_URL, '_blank')
                                    }
                                }}
                            >
                                {buttonType === 'try' ? t("pricing.plan.actions.try") :
                                    buttonType === 'buy' ? t("pricing.plan.button.buy") : t("pricing.contact.now")}
                            </Button>
                            {linkText && (
                                <a href={`${process.env.NEXT_PUBLIC_MUSEAI_URL}/settings/checkout/plan?product=${priceKey}`} target='_blank' className="mt-4 block font-mono text-sm">
                            ...
                            <ul className="mt-[30px] space-y-3">
                                {summary.map((feature) => (
                                    <li className="flex items-center gap-2 " key={feature}>
                                        ...
                                        <span className="font-mono text-[13px] font-light leading-[18px]">{feature}</span>
                                    </li>
                                ))}
